...
# Line endings are incorrect - need LF not CRLF
set -e
source ./config
...
#Convert to .raw 
# Would it not make sense to do this once rather than repeat per phenotype?
# Changed plink to "${plink}"
"${plink}" \
	--bfile "${bfile_raw}" \
	--allow-no-sex \
...
# This is assuming home directory is set correctly
